...
JustGage = function (config) {
  ...
  this.canvas.customAttributes.pki = function (value, min, max, w, h, dx, dy, gws) {
    ...
    var alpha = (1 - (value - min) / (max - min)) * Math.PI, Ro = w / 2 - w / 10, Ri = Ro - w / 6.666666666666667 * gws,
      ...
      Xo = w / 2 + dx + Ro * Math.cos(alpha), Yo = h - (h - Cy) + dy - Ro * Math.sin(alpha),
      Xi = w / 2 + dx + Ri * Math.cos(alpha), Yi = h - (h - Cy) + dy - Ri * Math.sin(alpha), path;
  ...
  // animate
  this.level.animate({
    pki: [this.config.value, this.config.min, this.config.max, this.params.widgetW,
